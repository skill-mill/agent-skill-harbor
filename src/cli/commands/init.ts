...
const cliPackageJson = JSON.parse(readFileSync(join(packageRoot, 'package.json'), 'utf-8')) as {
	version: string;
	harborCollectorRange?: string;
};
const packageRange = `>=${cliPackageJson.version} <1`;
const collectorRange = cliPackageJson.harborCollectorRange;
if (!collectorRange) {
	throw new Error('Missing harborCollectorRange in package.json');
}
...
function writeCollectorPackage(targetDir: string, projectName: string): void {
	copyTemplateFile(
		join(templatesDir, 'collector', 'package.template.json'),
		join(targetDir, 'collector', 'package.json'),
		projectName,
	);
	console.log('  Created collector/package.json');
}
...
export async function runCommand(argv = process.argv.slice(3)): Promise<void> {
	...
	try {
		...
		writeCollectorPackage(options.targetDir, projectName);
