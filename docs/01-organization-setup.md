...
4. The `AuditSkills` workflow will run automatically after collection
5. The deploy workflow will run automatically after audit
...
In `.github/workflows/`, enable `workflow_run` in `DeployCloudflarePages` and disable it in `DeployGitHubPages`. Both deploy workflows should listen to `AuditSkills`, and only one deploy workflow should have `workflow_run` enabled at a time.
