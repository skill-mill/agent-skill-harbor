...
built-in の `static` engine は、現在以下の観点で findings を分類します。
...
| `capability_risk`        | 破壊的操作や任意コード実行に近いパターン               | `rm -rf`, `eval(`, `exec(`, `child_process`, `os.system`, `subprocess`           | `fail`     | `2026-ASI03`, `2026-ASI05` |
| `permission_scope`       | 権限昇格や危険な権限前提                               | `sudo`, `chmod 777`, `--privileged`, `as root`                                   | `warn`     | `2026-ASI04`, `2026-ASI05` |
| `data_handling`          | 秘密情報や認証情報、機微なローカルファイルへのアクセス | `process.env`, `api_key`, `secret`, `token`, `password`, `.env`, `~/.ssh`        | `warn`     | `2026-ASI06`               |
| `external_communication` | 外部通信やデータ送信に関わるパターン                   | `curl`, `wget`, `fetch(`, `https://`, `webhook`                                  | `warn`     | `2026-ASI03`, `2026-ASI09` |
| `provenance_trust`       | 供給元やサプライチェーンに関する参照                   | `_from:`, `forked from`, `upstream`, `mirror`                                    | `warn`     | `2026-ASI07`, `2026-ASI09` |
...
- built-in の最終 `result` は、`fail` finding が1件でもあれば `fail`、そうでなければ finding が1件以上あると `warn` になります。
