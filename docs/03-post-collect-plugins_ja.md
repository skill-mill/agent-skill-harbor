...
- 生成される GitHub Actions workflow では、Harbor の reusable collect workflow が `config/harbor.yaml` からこの plugin を自動検出し、`post_collect` job で Python と `cisco-ai-skill-scanner` を install します
...
生成プロジェクトの reusable collect workflow は、`post_collect` job で `plugins/*/package.json` を検出し、各 plugin directory を install します。
...
その後、`config/harbor.yaml` の対応する plugin 設定をアンコメントしてください。
`notify-slack` は `plugins/notify-slack/package.json` も生成するため、ローカルで使う前に:
...
生成される `CollectSkills` workflow は、Harbor が publish している reusable workflow（`wf-v0`）を呼び出す薄い caller workflow です。

reusable workflow 側では、次の順で処理します。

1. `tools/harbor/collector` を install して `collect`
2. `tools/harbor/post-collect` を install
3. `plugins/*/package.json` があれば plugin ごとに install
4. `post-collect --collect-id ...` を実行
5. 更新された `data/` を commit/push

これにより、収集処理と post-collect 処理を分離して後段だけ再実行しやすくしつつ、生成プロジェクト側の workflow YAML を小さく保てます。
