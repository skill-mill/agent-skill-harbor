...
export async function ensureSkillScannerAvailable(command: string): Promise<void> {
	try {
		...
		throw new Error(
			[
				`skill-scanner CLI is unavailable: ${message}`,
				'Run `harbor setup builtin.audit-skill-scanner` and install collector/plugins/builtin.audit-skill-scanner/requirements.txt first.',
			].join(' '),
		);
	}
}
