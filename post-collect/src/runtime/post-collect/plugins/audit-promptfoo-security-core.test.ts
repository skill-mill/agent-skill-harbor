...
test('withPromptfooSafeEnv temporarily disables promptfoo telemetry and update checks', async () => {
	...
	await withPromptfooSafeEnv(async () => {
		assert.equal(process.env.PROMPTFOO_DISABLE_TELEMETRY, '1');
		assert.equal(process.env.PROMPTFOO_DISABLE_UPDATE, '1');
	});

	assert.equal(process.env.PROMPTFOO_DISABLE_TELEMETRY, previousDisableTelemetry);
	assert.equal(process.env.PROMPTFOO_DISABLE_UPDATE, previousDisableUpdate);
});
